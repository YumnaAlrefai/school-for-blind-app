...
class _TestQuestionsScreenState extends State<TestQuestionsScreen> {
  ...
  void _removeQuestion(int index) {
    if (_questions.length == 1) {
      _showMessage('يجب أن يحتوي الاختبار على سؤال واحد على الأقل');
      return;
    }
    setState(() {
  ...
  void _uploadTest() {
    ...
    final subjectId = getIt<TeacherLessonsCubit>().selectedSubject?.id;
    if (subjectId == null) {
    ...
    getIt<TeacherLessonsCubit>().emitCreateExam(body);
  }
  ...
  Widget build(BuildContext context) {
    return Directionality(
      ...
      child: Scaffold(
        ...
        body: SafeArea(
          child: BlocConsumer<TeacherLessonsCubit, ResultState<dynamic>>(
            bloc: getIt<TeacherLessonsCubit>(),
            listener: (context, state) {
              state.whenOrNull(
                success: (data) {
                  if (data == 'exam_created') {
                    _showMessage('تم إرسال الاختبار للإدارة للموافقة عليه');
                ...
                failure: (_) => _showMessage(
                  'فشل رفع الاختبار، تأكد من الاتصال وحاول مجدداً',
                ),
              );
            },
    ...
    return Center(
      child: SizedBox(
        ...
        child: ElevatedButton(
          onPressed: isUploading ? null : _uploadTest,
